...
const WebARRocksFaceShape2DHelper = (function () {
  ...
  let _videoElement = null,
    _videoElementPreviousTime = -1
  let _gl = null,
    _glVideoTexture = null // gl context is for the AR canvas
  let _glv = null,
    _glvVideoTexture = null // glv is for video and computation
  ...
  function callbackTrack(detectState) {
    ...
    if (detectState.isDetected) {
      const landmarksStabilized = _landmarksStabilizer.update(
  ...
  function init_gl() {
    ...
    _gl.blendFunc(_gl.SRC_COLOR, _gl.ONE_MINUS_SRC_ALPHA)
  }
  ...
  function init_shps() {
    ...
    _shps.drawVideo = build_shaderProgram(
      _glv,
    ...
    _shps.drawVideo.uniforms.transformMat2 = _glv.getUniformLocation(
      _shps.drawVideo.program,
      'transform'
    )
  }
  ...
  function build_shape(landmarkLabels, shapeSpecsArg, shapeIndex) {
    ...
    return new Promise(function (accept, reject) {
      ...
      if (isIVals) {
        ...
        const GLSLType = ['float', 'vec2', 'vec3', 'vec4'][iValCCount - 1]
      ...
      const texturesPromise =
        texturesPromises.length === 0 ? Promise.resolve([]) : Promise.all(texturesPromises)
      ...
      const fragmentShaderSource =
        iValsShaderSources.fragmentPars +
        '\n' +
      ...
      const shp = build_shaderProgram(
        _gl,
        vertexShaderSource,
        fragmentShaderSource,
        'SHAPE_' + shapeIndex.toString()
      )
      ...
      _gl.bufferData(
        ...
        new Uint16Array(shapeSpecs.tesselation),
        _gl.STATIC_DRAW
      )
  ...
  function split_interpolations(interpolationSpecs) {
    ...
    const sortByAbs = function (a, b) {
      // sort from smaller to larger in abs value
  ...
  function build_interpolation(shapeSpecs, interpolationSpecs, interpolationInd) {
    ...
    for (let i = 0; i < tess.length; i += 3) {
      ...
      for (let i = 0; i < ks.length; ++i) {
        ...
        if (ks[i] > 0) {
          // keep face indexing order for backface culling:
  ...
  async function draw_shape(landmarksPositions, shape) {
    if (_spec.updateCallback) {
      if ((await _spec.updateCallback(shape)) === 'pause') {
        return
      }
    }
    ...
    _gl.uniformMatrix2fv(
      shape.shp.uniforms.videoUVScale,
      false,
      WEBARROCKSFACE.get_videoUVScaleMat2()
    )
        ...
        if (p[1] <= Math.max(p1[1], p2[1])) {
          if (p[0] <= Math.max(p1[0], p2[0])) {
            if (p1[1] !== p2[1]) {
              ...
              if (p1.x === p2[0] || p[0] <= xinters) ++counter
            }
          }
        }
      }
  ...
  function apply_outline(pointPositions, outline) {
    ...
    const w = _spec.canvasAR.width,
      h = _spec.canvasAR.height
  ...
  function compute_interpolation(pointPositions, interpolation) {
    ...
    const m0 = interpolation.m0,
      m1 = interpolation.m1
    m0[0] =
      pointPositions[2 * interpolation.points[2]] - pointPositions[2 * interpolation.points[0]]
    m0[1] =
      pointPositions[2 * interpolation.points[2] + 1] -
      pointPositions[2 * interpolation.points[0] + 1]
    ...
    return new Promise(function (accept, reject) {
      const initSettings = {
        ...
        callbackReady: function (err, objs) {
          ...
          init_shps()
          Promise.all(_spec.shapes.map(build_shape.bind(null, objs.landmarksLabels))).then(
            function (shapes) {
              _shapes = shapes
              accept()
            }
